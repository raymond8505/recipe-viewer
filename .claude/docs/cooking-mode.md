...
- The primary recipe is held as one `doc: RecipeDocument` (`recipeDocument(recipe)` — schema, ingredient groups, instruction groups, time columns), built once when cook mode opens; `activeScalable = scalables.get(mealRecipes[activeIndex].id)` is what drives ingredients rendering (`groupedIngredients`), `activeScalable.schema` drives the notes, and `activeInstructions` — `doc.instructions` for the primary, `mealRecipes[i].instructions` for a secondary — drives the steps. Nutrition is derived from `doc.ingredients` (`recipeNormalizedNutrition`), primary recipe only — added recipes come from list rows that carry no catalog data
