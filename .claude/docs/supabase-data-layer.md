...
- `getRecipes` hydrates rows only, in one batched query per page (`getRecipeIngredientsByRecipeIds`, chunked by 100 ids for the PostgREST URL limit): `ingredient` is left **undefined**. The rows are not optional — `/api/recipes` feeds MealSearch, whose rows go straight into a `ScalableRecipe` when a recipe joins a meal — but nothing on a list page computes nutrition, and `computeRecipeNutrition` treats undefined and null alike, so a list row can never compute a total by accident.
