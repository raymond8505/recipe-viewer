...
- **`checkbox.tsx` diverges out of necessity, not taste:** the registry file styles its checked state with `data-checked:` variants, which target a newer Radix than the pinned `@radix-ui/react-checkbox` 1.3.5 (via the `radix-ui` umbrella package) — that version only emits `data-state="checked|unchecked|indeterminate"`, so the registry selectors never match and a checked box renders as an empty outline. Ours uses `data-[state=…]:` and adds a `MinusIcon` for the indeterminate state (the registry indicator only ever renders a check, which reads as "all selected" on a partial group). Re-apply both on any regeneration; verify by grepping the built CSS for `[data-state=checked]`.
