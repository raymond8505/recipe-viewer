...
- **Row config is `src/components/nutritionLinearRows.ts`** (`LINEAR_NUTRIENT_ROWS`, ordered by `LINEAR_NUTRIENT_ORDER` via `exhaustiveKeys` — an 11th `NutrientField` is a compile error). It carries **no `unit`**, unlike `ingredients/nutritionFacts.ts`'s `NUTRITION_LABEL_ROWS`: a `NutrientValue` brings its own and `formatNutrientDisplay` re-attaches it.
- **Two FDA labels exist and stay separate.** `ingredients/NutritionFactsLabel` is the catalog editor's vertical panel keyed on `IngredientNutrition` (snake_case, has calcium/iron/potassium, no unsaturated fat); `NutritionLinearLabel` is the reader's horizontal one keyed on `NutrientField`. Converting is lossy both ways — don't merge them. The FDA wording they share is held in lockstep by `src/__tests__/nutritionLinearRows.test.ts`; the two deliberate exclusions (`calories`, `unsaturatedFatContent`) are asserted there too, so a newly-shared nutrient can't slip past unchecked.
- **`NutritionLinearLabel` takes `NutrientValues`, not `ScaledNutrition`, on purpose** — `servingSize` rides through `scaleNutrition` unscaled, so it lies once the stepper moves. The prop type makes the mistake unavailable; use `servingLabel` (= `nutritionUnitLabel`).
- calcium/iron/potassium stay out of the label. They have no Schema.org slot (`SCHEMA_NUTRITION_MAP` drops them), so plumbing them through would make the label's field set vary by source — only the ingredients view could ever supply them, which fights the all-or-nothing gate.
- Micro-gotchas: the label's `{" "}` around each separator is load-bearing (it's what creates the soft-wrap opportunities); "Nutrition Facts" is a `<div>` in both labels so the serif-light heading base layer doesn't catch it; in label view `nutritionUnitLabel` renders **twice** (header + serving line), so tests must use `getAllByText`.
