...
- **Reading or writing `SchemaRecipe`, a recipe's ingredients or its instructions** — `RecipeIngredientGroup`/`RecipeIngredient`, `RecipeInstructionGroup`/`RecipeStep`, the Schema.org edges (`toSchemaOrgJsonLd`/`fromSchemaOrgIngredients`/`fromSchemaOrgInstructions`), the servings columns and `parseYield` → [docs/recipe-schema.md](docs/recipe-schema.md)
