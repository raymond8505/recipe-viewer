...
- Micro-gotchas: **Calories is the big display number and drops its unit** in both layouts, so tests assert `"520"`, not `"520 kcal"`; "Nutrition Facts" is a `<div>` so the serif-light heading base layer doesn't catch it; in label view `nutritionUnitLabel` renders **twice** (panel header + label serving line), so tests must use `getAllByText`. Coverage/naming is pinned in `src/__tests__/nutritionLabelRows.test.ts` (both adapters, against `NUTRIENT_FIELDS` and `NUTRITION_COLUMNS`).
