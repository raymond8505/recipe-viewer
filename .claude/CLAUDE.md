...
**NutritionPanel's summary/label toggle is a layout swap, not a second source.** The panel's `view` state (`"summary" | "label"`, session-only, defaults to `"summary"`) picks between the curated six-stat grid and `NutritionLinearLabel` — the reader-facing FDA "linear display" label. Both render the *same* `nutrition()!.values` on the *same* basis (`nutritionUnitLabel` + the portion stepper); the label just shows all ten `NutrientField`s instead of six, absent ones as an em dash. Never give the label its own derivation, its own basis, or whole-recipe totals.
...
- **The three minerals are always null on the recipe side.** They have no Schema.org slot (`SCHEMA_NUTRITION_MAP` drops them), so that path can never fill them; they render as em dashes rather than being dropped, to keep the label's shape. Don't "fix" this by widening `NutrientField` — only the ingredients view could ever supply them, so the label's field set would then vary by source, fighting the all-or-nothing gate. Conversely the ingredient adapter **omits** the unsaturated-fat row entirely (no catalog column) rather than showing a permanently-empty line.
