...
export const recipeImageUploadInputSchema = z
  .object({
    ...
    // Base64 in MCP tool arguments is generated by the model as output
    // tokens, so multi-MB payloads cost minutes before the request even
    // arrives. Cap well below MAX_IMAGE_BYTES and name the fast paths in
    // the error so the caller can recover without guessing.
    imageBase64: z
      .string()
      .min(1)
      .max(1_400_000, {
