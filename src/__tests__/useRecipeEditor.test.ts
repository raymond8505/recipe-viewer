...
describe("useRecipeEditor", () => {
  ...
  it("begin seeds yieldFields from an object-form recipeYield", () => {
    const { result } = renderHook(() => useRecipeEditor());
    act(() =>
    ...
    expect(result.current.draft.yieldFields).toEqual({
      servings: "4",
      unit: "kebabs",
      weight: "454",
      weightUnit: "g",
  ...
  it("buildSchema builds a QuantitativeValue from the yield fields", () => {
    const { result } = renderHook(() => useRecipeEditor());
    act(() => result.current.begin(schema, "draft", ""));
    act(() =>
  ...
  it("migrates a legacy string yield to a QuantitativeValue on save", () => {
    const withString: SchemaRecipe = { ...schema, recipeYield: "4 servings" };
    const { result } = renderHook(() => useRecipeEditor());
    act(() => result.current.begin(withString, "draft", ""));
    expect(result.current.draft.yieldFields).toMatchObject({
      servings: "4",
      unit: "servings",
    });
    expect(result.current.buildSchema(withString).recipeYield).toEqual({
      "@type": "QuantitativeValue",
      value: 4,
      unitText: "servings",
    });
